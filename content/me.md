---
socials:
  - platform: github
    url: https://github.com/alvarosabu
  - platform: twitter
    url: https://twitter.com/alvarosabu
  - platform: bluesky
    url: https://bsky.app/profile/alvarosaburido.dev
  - platform: youtube
    url: https://www.youtube.com/@AlvaroDevLabs
  - platform: discord
    url: https://discord.gg/UCr96AQmWn
  - platform: linkedin
    url: https://linkedin.com/in/alvarosaburido
sponsors:
  github: https://github.com/sponsors/alvarosabu
  tresjs: https://github.com/sponsors/Tresjs
---

# Hey, I'm Alvaro

Creative Software Engineer

Working at :magic-link{label="Directus"}

Creator of :magic-link{label="TresJS"} - bringing 3D to :magic-link{label="Vue"}

Previously at :magic-link{label="Storyblok"}, :magic-link{label="Porsche Digital"}

---

I believe the web should be a canvas for creativity. My goal is enabling developers to build the creative stuff they dream of but never thought was possible. 

---

I'm a public speaker, you can check some of my talks [here](/talks). I also write [blog posts](/blog) about creative web development and my personal journey. Occasionally, I create content at  :magic-link{label="AlvaroDevLabs"} about creative web development and my personal journey.

When not coding, I enjoy everything that can be considered creative. I'm a big fan of photography (currently learning), videography, gaming. If not that, you can find me scuba diving 🤿, carving down snowy mountains 🏂, or getting lost in :magic-link{label="Blender"} building 3D worlds.

I share my life in Terrassa, Barcelona with two cats: **Geralt** (yes, the Witcher) and **Mochi** (the Japanese dessert). Originally from Venezuela 🇻🇪
